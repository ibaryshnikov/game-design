use iced::{Application, Command, Subscription, Element, Theme, Length, Settings, Alignment};
use serde::{Deserialize, Serialize};
use iced::widget::{button, text, container, column, row, text_input, pick_list};

use shared::attack::{AttackConstructor, AttackOrder, AttackKind};
...
fn read_file() -> Option<AttackConstructor> {
    let contents = std::fs::read("data/attack.json")
        .ok()?;
...
fn write_file(attack: &Option<AttackConstructor>) {
    ...
    let contents = serde_json::to_vec(attack)
        .expect("Should encode a AttackConstructor");
    std::fs::write("data/attack.json", contents)
        .expect("Should write AttackConstructor to a file");
}
    ...
    fn update(&mut self, message: Message) -> Command<Message> {
        match message {
            ...
            Message::ChangeDelay(value) => {
                ...
                let Some(parsed) = parsed else { return Command::none() };
    ...
    fn view(&self) -> Element<Message> {
        ...
        if let Some(attack) = &self.attack {
            ...
            let attack_details_column = column![
                ...
                row![
                    text("Order"),
                    pick_list(AttackOrder::options(), Some(attack.order.clone()), Message::ChangeOrder)
                ...
                row![
                    text("Kind"),
                    pick_list(AttackKind::options(), Some(attack.kind.clone()), Message::ChangeKind)
                ...
                .spacing(10);
            let attack_details = container(attack_details_column)
                .width(300);
